...
- Drizzle schemas live in `packages/core/src/**/*.sql.ts`; core owns and applies its database migrations.

## Development server

- `bun dev` from this package starts an interactive TUI. Use a persistent terminal/PTY or another environment-supported session so its output remains inspectable and it does not block the task.
- Stop development sessions created for the task when finished; preserve user-managed processes.

## Module shape

- Use flat ESM exports with the existing self-reexport convention, not `export namespace`.
- For module-layout changes, consult [module conventions](specs/agent-module-conventions.md). Multi-sibling directories use direct sibling imports, not a new barrel.
